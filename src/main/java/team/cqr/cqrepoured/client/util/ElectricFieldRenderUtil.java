...
public class ElectricFieldRenderUtil {
	...
	public static void renderElectricField(Entity entity, double x, double y, double z, int bolts) {
		...
		for (int boltCount = 0; boltCount < bolts; ++boltCount) {
			...
			for (int i = 0; i <= steps; ++i) {
				double distance = entity.width;
				double boltSize = 80;

				double vX = Math.sin((i + rng.nextInt()) / boltSize * Math.PI * 2.0D) * distance + (rng.nextDouble() - 0.5D) * (entity.width / 2);
				double vZ = Math.cos((i + rng.nextInt()) / boltSize * Math.PI * 2.0D) * distance + (rng.nextDouble() - 0.5D) * (entity.width / 2);
