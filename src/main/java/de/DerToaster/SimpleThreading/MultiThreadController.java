...
public class MultiThreadController {
	...
	public void addTask(Runnable task) {
		...
		/*
		 * boolean taskNotAdded = true; while(taskNotAdded) { for(SimpleThread st : this.threads) { //if(!st.isQueueLocked()) { st.addTask(task); taskNotAdded = false; System.out.println("Task added to thread " + st.getName()); break; //} }
		 * }
		 */
		// if(this.tasksToAdd.size() >= this.threads.size()) {
		/*
		 * int threadIndex = 0; while(!this.tasksToAdd.isEmpty()) { if(threadIndex >= this.threads.size()) threadIndex = 0;
		 * 
		 * Runnable taskToAdd = this.tasksToAdd.remove();
		 * 
		 * SimpleThread st = this.threads.get(threadIndex); st.addTask(taskToAdd); System.out.println("Task added to thread " + st.getName());
		 * 
		 * threadIndex++; }
		 */
		// }
