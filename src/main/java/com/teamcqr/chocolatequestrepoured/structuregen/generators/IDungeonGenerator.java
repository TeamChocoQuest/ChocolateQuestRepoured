...
public interface IDungeonGenerator {
	...
	default void generate(World world, Chunk chunk, int x, int y, int z) {
		/*
		 * int median = 0; int cant = 0; for(int iX = 0; iX < x; iX++) { for(int iZ = 0; iZ < z; iZ++) { int height = world.getTopSolidOrLiquidBlock(new BlockPos(iX, 0, iZ)).getY(); median += height; cant++; } } y = median /cant;
		 */
		if (world.isRemote) {
			return;
		}

		this.preProcess(world, chunk, x, y, z);
		this.buildStructure(world, chunk, x, y, z);
		this.postProcess(world, chunk, x, y, z);
		this.fillChests(world, chunk, x, y, z);
		this.placeSpawners(world, chunk, x, y, z);
		this.placeCoverBlocks(world, chunk, x, y, z);
	}
