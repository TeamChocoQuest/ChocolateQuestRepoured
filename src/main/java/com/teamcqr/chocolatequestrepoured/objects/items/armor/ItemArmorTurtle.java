...
public class ItemArmorTurtle extends ItemArmor {
	...
	public void onArmorTick(World worldIn, EntityPlayer player, ItemStack stack) {
		if (ItemUtil.isFullSet(player, stack)) {
			...
			if (ticksFromLastHeal == 1200) {
				...
				if (worldIn.isRemote) {
					worldIn.spawnParticle(EnumParticleTypes.HEART, player.posX + itemRand.nextDouble() - 0.5D,
							player.posY + itemRand.nextDouble(), player.posZ + itemRand.nextDouble() - 0.5D, 0.0D, 1.0D,
							0.0D);
				}
			}
		}

		if (player.getItemStackFromSlot(EntityEquipmentSlot.HEAD).getItem() == ModItems.HELMET_TURTLE
				&& stack.getItem() == ModItems.HELMET_TURTLE) {
			...
			if (helmet.hasKey(CD) && helmet.hasKey(ON)) {
				...
				if (player.getHealth() < 2.0F && helmet.getInteger(CD) == 0 && ItemUtil.isFullSet(player, stack)
						&& isFullSetReady(player)) {
					helmet.setBoolean(ON, true);
				}

				if (helmet.getBoolean(ON) && ItemUtil.isFullSet(player, stack) && isFullSetReady(player)) {
					if (player.getHealth() < player.getMaxHealth()) {
						heal(player, 1.0F);

						if (worldIn.isRemote) {
							worldIn.spawnParticle(EnumParticleTypes.HEART, player.posX + itemRand.nextDouble() - 0.5D,
									player.posY + itemRand.nextDouble(), player.posZ + itemRand.nextDouble() - 0.5D,
									0.0D, 1.0D, 0.0D);
						}
					} else {
						helmet.setBoolean(ON, false);
		...
		if (player.getItemStackFromSlot(EntityEquipmentSlot.CHEST).getItem() == ModItems.CHESTPLATE_TURTLE
				&& stack.getItem() == ModItems.CHESTPLATE_TURTLE) {
			...
			if (chestplate.hasKey(CD) && chestplate.hasKey(ON)) {
				...
				if (player.getHealth() < 2.0F && chestplate.getInteger(CD) == 0 && ItemUtil.isFullSet(player, stack)
						&& isFullSetReady(player)) {
					chestplate.setBoolean(ON, true);
				}

				if (chestplate.getBoolean(ON) && ItemUtil.isFullSet(player, stack) && isFullSetReady(player)) {
					if (player.getHealth() < player.getMaxHealth()) {
						heal(player, 1.0F);
		...
		if (player.getItemStackFromSlot(EntityEquipmentSlot.LEGS).getItem() == ModItems.LEGGINGS_TURTLE
				&& stack.getItem() == ModItems.LEGGINGS_TURTLE) {
			...
			if (leggings.hasKey(CD) && leggings.hasKey(ON)) {
				...
				if (player.getHealth() < 2.0F && leggings.getInteger(CD) == 0 && ItemUtil.isFullSet(player, stack)
						&& isFullSetReady(player)) {
					leggings.setBoolean(ON, true);
				}

				if (leggings.getBoolean(ON) && ItemUtil.isFullSet(player, stack) && isFullSetReady(player)) {
					if (player.getHealth() < player.getMaxHealth()) {
						heal(player, 1.0F);
		...
		if (player.getItemStackFromSlot(EntityEquipmentSlot.FEET).getItem() == ModItems.BOOTS_TURTLE
				&& stack.getItem() == ModItems.BOOTS_TURTLE) {
			...
			if (boots.hasKey(CD) && boots.hasKey(ON)) {
				...
				if (player.getHealth() < 2.0F && boots.getInteger(CD) == 0 && ItemUtil.isFullSet(player, stack)
						&& isFullSetReady(player)) {
					boots.setBoolean(ON, true);
				}

				if (boots.getBoolean(ON) && ItemUtil.isFullSet(player, stack) && isFullSetReady(player)) {
					if (player.getHealth() < player.getMaxHealth()) {
						heal(player, 1.0F);
	...
	private void applyCooldown(EntityPlayer player) {
		if (player.getItemStackFromSlot(EntityEquipmentSlot.HEAD).getItem() == ModItems.HELMET_TURTLE
				&& player.getItemStackFromSlot(EntityEquipmentSlot.CHEST).getItem() == ModItems.CHESTPLATE_TURTLE
				&& player.getItemStackFromSlot(EntityEquipmentSlot.LEGS).getItem() == ModItems.LEGGINGS_TURTLE
				&& player.getItemStackFromSlot(EntityEquipmentSlot.FEET).getItem() == ModItems.BOOTS_TURTLE) {
	...
	private boolean isFullSetReady(EntityPlayer player) {
		if (player.getItemStackFromSlot(EntityEquipmentSlot.HEAD).getItem() == ModItems.HELMET_TURTLE
			...
			if (helmet.getInteger(CD) > 0 || chestplate.getInteger(CD) > 0 || leggings.getInteger(CD) > 0
					|| boots.getInteger(CD) > 0) {
				return false;
			}
		}
		return true;
		...
		if (stack.hasTagCompound()) {
			...
			if (tag.hasKey(CD)) {
				if (tag.getInteger(CD) > 0) {
					tooltip.add(TextFormatting.RED + I18n.format("description.turtle_armor_charging.name")
							+ convertCooldown(stack));
				}
			}
		}
