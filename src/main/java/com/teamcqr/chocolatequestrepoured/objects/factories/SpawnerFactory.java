	...
	public static void placeSpawner(NBTTagCompound[] entities, boolean multiUseSpawner, @Nullable NBTTagCompound spawnerSettingsOverrides, World world, BlockPos pos) {
		...
		if (multiUseSpawner) {
			...
			for (int i = 0; i < entities.length; i++) {
				if (entities[i] != null) {
					/*
					 * NBTTagCompound entityToAddAsNBT = new NBTTagCompound(); entityToAddAsNBT.setString("id", EntityList.getEntityString(entities[i])); spawnerEntities.set(i, entityToAddAsNBT);
					 */
					// This could actually work
					/*
					 * NBTTagCompound entityTag = new NBTTagCompound(); entityTag.setTag("Entity", entities[i].writeToNBT(new NBTTagCompound())); spawnerEntities.appendTag(entityTag);
					 */
