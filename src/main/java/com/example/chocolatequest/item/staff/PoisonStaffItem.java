...
public class PoisonStaffItem extends Item implements IRangedWeapon {

    public PoisonStaffItem(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (player.getFoodData().getFoodLevel() >= 1 || player.getAbilities().instabuild) {
            ...
            if (!level.isClientSide()) {
                PoisonProjectileEntity projectile = new PoisonProjectileEntity(level, player);
                projectile.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 1.5F, 1.0F);
                level.addFreshEntity(projectile);
                
                level.playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.SLIME_ATTACK, SoundSource.PLAYERS, 0.5F, 0.4F / (level.getRandom().nextFloat() * 0.4F + 0.8F));
            }
    ...
    public void shoot(net.minecraft.world.level.Level level, net.minecraft.world.entity.LivingEntity shooter, net.minecraft.world.entity.Entity target, net.minecraft.world.InteractionHand hand) {
        ...
        if (!level.isClientSide()) {
            com.example.chocolatequest.entity.projectile.PoisonProjectileEntity projectile = new com.example.chocolatequest.entity.projectile.PoisonProjectileEntity(level, shooter);
    ...
    public net.minecraft.sounds.SoundEvent getShootSound() {
        return net.minecraft.sounds.SoundEvents.SLIME_ATTACK;
    }
