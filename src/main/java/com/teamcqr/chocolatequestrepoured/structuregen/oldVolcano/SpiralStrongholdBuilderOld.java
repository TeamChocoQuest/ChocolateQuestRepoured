...
public class SpiralStrongholdBuilderOld {
	...
	private SpiralStrongholdFloorOld[] floors;
	private int floorCount = 0;
	private List<AbstractDungeonPart> strongholdParts = new ArrayList<>();
	private Random rdm;
	
	public SpiralStrongholdBuilderOld(AbstractDungeonGenerator<DungeonVolcanoOld> generator, DungeonGenerator dungeonGenerator, ESkyDirection expansionDirection, DungeonVolcanoOld dungeon, Random rdm) {
		...
		this.floors = new SpiralStrongholdFloorOld[floorCount];
	}
	...
	public void calculateFloors(BlockPos strongholdEntrancePos) {
		...
		for(int i = 0; i < floors.length; i++) {
			...
			SpiralStrongholdFloorOld floor = new SpiralStrongholdFloorOld(this.generator, this.dungeonGenerator, posTuple, entranceX, entranceZ, roomCount <= 0 || i == (floors.length -1), dungeon.getFloorSideLength(), floorRoomCount);
	...
	public void buildFloors(BlockPos strongholdEntrancePos, World world, int dungeonChunkX, int dungeonChunkZ, String mobType) {
		...
		for(int i = 0; i < floorCount; i++) {
			SpiralStrongholdFloorOld floor = this.floors[i];
