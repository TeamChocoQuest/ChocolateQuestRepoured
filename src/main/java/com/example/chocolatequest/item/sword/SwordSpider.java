...
public class SwordSpider extends CQSwordItem {
    public SwordSpider(Tier pTier, Properties pProperties) {
    ...
    public boolean hurtEnemy(ItemStack pStack, LivingEntity pTarget, LivingEntity pAttacker) {
        if (!pAttacker.level().isClientSide()) {
            pTarget.addEffect(new MobEffectInstance(MobEffects.POISON, 100, 1)); // 5 seconds Poison II
        }
