...
public class WaterStaffItem extends Item implements IRangedWeapon {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (player.getFoodData().getFoodLevel() >= 1 || player.getAbilities().instabuild) {
            ...
            if (!level.isClientSide()) {
                BubbleProjectileEntity projectile = new BubbleProjectileEntity(level, player);
                projectile.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 1.0F, 1.0F);
                level.addFreshEntity(projectile);
                
                level.playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.GENERIC_SPLASH, SoundSource.PLAYERS, 0.5F, 0.4F / (level.getRandom().nextFloat() * 0.4F + 0.8F));
            }
    ...
    public void shoot(net.minecraft.world.level.Level level, net.minecraft.world.entity.LivingEntity shooter, net.minecraft.world.entity.Entity target, net.minecraft.world.InteractionHand hand) {
        ...
        if (!level.isClientSide()) {
            com.example.chocolatequest.entity.projectile.BubbleProjectileEntity projectile = new com.example.chocolatequest.entity.projectile.BubbleProjectileEntity(level, shooter);
            // aim at target
            double dx = target.getX() - shooter.getX();
            double dy = target.getY(0.3333333333333333D) - projectile.getY();
            double dz = target.getZ() - shooter.getZ();
            double d = Math.sqrt(dx * dx + dz * dz);
            projectile.shoot(dx, dy + d * 0.2D, dz, 1.0F, 1.0F);
            level.addFreshEntity(projectile);
