...
public class ItemStaffPoison extends ItemLore implements IRangedWeapon {
	...
	public void shoot(ItemStack stack, Level worldIn, Player player, InteractionHand handIn) {
		worldIn.playLocalSound(player.position().x, player.position().y, player.position().z, SoundEvents.SNOWBALL_THROW, SoundSource.MASTER, 4.0F, (1.0F + (random.nextFloat() - random.nextFloat()) * 0.2F) * 0.7F, false);
		player.swing(handIn);
