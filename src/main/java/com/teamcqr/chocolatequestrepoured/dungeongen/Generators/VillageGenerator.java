...
public class VillageGenerator implements IDungeonGenerator{
	...
	public void buildStructure(World world, Chunk chunk, int x, int y, int z) {
		...
		platformCenter.generateHill(new Random(), world, x /*- (centerDun.getSizeX() /2)*/, y + this.dungeon.getUnderGroundOffset(), z /*- (centerDun.getSizeZ() /2)*/, centerDun.getSizeX(), centerDun.getSizeZ());
		...
		for(int i = 0; i < this.structurePosList.size(); i++) {
			if(i < this.chosenStructures.size()) {
				...
				if(dungeonToSpawn != null) {
					...
					platform.generateHill(new Random(), world, pos.getX() /*- (dungeonToSpawn.getSizeX() /2)*/, pos.getY() + this.dungeon.getUnderGroundOffset(), pos.getZ() /*- (dungeonToSpawn.getSizeZ() /2)*/, dungeonToSpawn.getSizeX(), dungeonToSpawn.getSizeZ());
