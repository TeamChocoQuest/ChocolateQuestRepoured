...
public class NetherCityHangingGenerator implements IDungeonGenerator {
	...
	private BlockPos getNextIslandPos(BlockPos centerPos, int islandIndex) {
		 ...
		 * 
		 * while(!structureMap.isEmpty() || !structureMap.containsKey(retPos) || locIsNotFine(retPos)) { //DONE: Calculate new position retPos = getNextIslandPos(retPos, islandIndex++); }
		 */
	...
	/*
	 * private boolean locIsNotFine(BlockPos pos) { for(BlockPos p : structureMap.keySet()) { double dist = pos.getDistance(p.getX(), pos.getY(), p.getZ()); dist = Math.abs(dist); if(dist < dungeon.getMinIslandDistance() || dist >
	 * dungeon.getMaxIslandDistance() || p.equals(pos)) { return true; } } return false; }
	 */
	...
	private void buildBuilding(CQStructure structure, BlockPos pos, World world, Chunk chunk) {
		...
		/*
		 * PlateauBuilder builder = new PlateauBuilder(); builder.createCave(new Random(), pos, pos.add(structure.getSizeX(), structure.getSizeY(), structure.getSizeZ()), world.getSeed(), world);
		 */
		...
		/*
		 * CQDungeonStructureGenerateEvent event = new CQDungeonStructureGenerateEvent(this.dungeon, pos, new BlockPos(structure.getSizeAsVec()), world); if(structure != null && structure.getShieldCorePosition() != null) {
		 * event.setShieldCorePosition(structure.getShieldCorePosition()); } MinecraftForge.EVENT_BUS.post(event);
		 */
	}
