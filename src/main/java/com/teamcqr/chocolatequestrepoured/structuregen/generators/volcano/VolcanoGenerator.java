...
public class VolcanoGenerator implements IDungeonGenerator {
	...
	/**
	 * Generate: Given height, given base radius, given top inner radius steepness: In % level begins at 0!! DONE: Outer Radius -> make new function, current function dows not really work out Outer Radius --> y=(level * steepness) * level^2
	 * --> y = steepness *level^3 --> RADIUS = baseRAD - (level/steepness)^1/3 Inner Radius --> given, OR if (OuterRadius - (maxHeight - currHeight)) > given && (OuterRadius - (maxHeight - currHeight)) < maxInnerRad then use (OuterRadius -
	 * (maxHeight - currHeight)) Block placing: generate circles. Per outer layer, the probability to place a block is reduced, same for height. A block can only be placed if it has a support block probability for blocks: max rad +1: 0%
	 * minRad -1: 100% --> P(x) = 1- [ (x-MIN)/(MAX-MIN) -steepNess * level] x = radius of block -> distance to center - innerRadius
	 * 
	 * 
	...
	public void placeSpawners(World world, Chunk chunk, int x, int y, int z) {
		...
		for (BlockPos pos : this.spawnersNChestsOnPath) {
			 ...
			 * 
			 * spawner.getSpawnerBaseLogic().setEntityId(this.dungeon.getMob()); //System.out.println("Spawner Mob: " + this.dungeon.getMob().toString()); spawner.updateContainingBlockInfo();
			 * 
			 * spawner.update();
			 */
