 ...
 *
 * This is an enum to map a handful of common block types to their associated blockstate in Forge 1.12. For example, chiseled stone bricks are not a Block type
 * in 1.12, rather they are a special variant state of STONE_BRICKS that cannot be looked up
 * by a namespace ID alone. Trying to parse through all the blocks and their property values is needlessly tedious, so this is just a shortcut so players can
 * use these block varieties in config files as if they had their own namespace ID.
 *
 * Note: I named the enum values after the block namespace IDs on the minecraft Wiki so that if we move to a later version of forge, the config values should
 * just map to registry values instead of coming here.
 */
