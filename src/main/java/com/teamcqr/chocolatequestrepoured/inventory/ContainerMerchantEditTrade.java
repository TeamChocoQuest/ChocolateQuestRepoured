...
public class ContainerMerchantEditTrade extends Container {
	...
	public void onContainerClosed(EntityPlayer playerIn) {
		 ...
		 * for (int i = 0; i < 4; i++) {
		 * playerIn.dropItem(this.tradeInventory.removeStackFromSlot(i), false);
		 * }
		 * } else {
		 * for (int i = 0; i < 4; i++) {
		 * playerIn.inventory.placeItemBackInInventory(playerIn.world, this.tradeInventory.removeStackFromSlot(i));
		 * }
		 * }
		 */
	}
