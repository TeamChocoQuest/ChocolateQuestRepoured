	...
	public void updateTask() {
		...
		if (this.cooldown >= COOLDOWN_BORDER) {
			...
			if (this.talkingPartner == null || this.cooldwonForPartnerCycle >= COOLDOWN_FOR_PARTNER_CYCLE_BORDER) {
				...
				/**
				 * Before PR Meldex //DONE: Make entity sit -> Renderer needs work for that //int friendsFound = 0; List<Entity> friends = new ArrayList<>(); List<Entity> nearbyEntities =
				 * entity.getEntityWorld().getEntitiesInAABBexcluding(entity, new AxisAlignedBB(entity.getPosition().subtract(new Vec3i(6,3,6)), entity.getPosition().add(6,3,6)), AbstractEntityCQR.MOB_SELECTOR);
				 * //System.out.println("NearbyEntities: " + nearbyEntities.size()); if(!nearbyEntities.isEmpty()) { for(Entity ent : nearbyEntities) { if(ent instanceof AbstractEntityCQR) {
				 * if(entity.getFaction().isAlly((AbstractEntityCQR)ent)) { friends.add(ent); //friendsFound++; } } } //System.out.println("Friend list size: " + friends.size()); if(!friends.isEmpty()) { cooldwonForPartnerCycle++;
				 * if(talkingPartner == null || cooldwonForPartnerCycle >= cooldownCyclePartnerBorder || talkingPartner.isDead || entity.getDistance(talkingPartner) > 8) { talkingPartner = friends.get(random.nextInt(friends.size()));
				 * //System.out.println("I found a partner!"); cooldwonForPartnerCycle = 0; } if(talkingPartner != null && talkingPartner != entity && !talkingPartner.isDead) { //we have someone to talk to, yay :D //DONE: Orient entity to
				 * random friend //DONE: Make them "chat" / "play cards" entity.setChatting(true); entity.getLookHelper().setLookPosition(talkingPartner.posX, talkingPartner.posY + talkingPartner.getEyeHeight(), talkingPartner.posZ,
				 * (float)this.entity.getHorizontalFaceSpeed(), (float)this.entity.getVerticalFaceSpeed());
				 * 
				 * } else { entity.setChatting(false); } } else { entity.setChatting(false); } END_OF_OLD_STATE
				 **/
			}
