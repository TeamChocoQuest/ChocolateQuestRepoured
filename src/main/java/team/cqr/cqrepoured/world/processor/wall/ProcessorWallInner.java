...
public class ProcessorWallInner extends StructureProcessor {
	
	public static final ProcessorWallInner INSTANCE = new ProcessorWallInner();
	public static final Codec<ProcessorWallInner> CODEC = Codec.unit(() -> INSTANCE);

	@Override
	protected IStructureProcessorType<?> getType() {
		return CQRStructureProcessors.PROCESSOR_WALL_INNER;
	}
	...
	public BlockInfo process(IWorldReader worldReader, BlockPos jigsawPos, BlockPos jigsawPieceBottomCenterPos, BlockInfo blockInfoLocal, BlockInfo blockInfoGlobal, PlacementSettings structurePlacementData, Template template) {
		if (blockInfoGlobal.state.is(Blocks.BLACK_STAINED_GLASS) && blockInfoLocal.pos.getY() == 0) {
