...
public class StrongholdLinearGenerator implements IDungeonGenerator {
	...
	public int getDunX() {return dunX;}
	public int getDunZ() {return dunZ;}
	...
	public void preProcess(World world, Chunk chunk, int x, int y, int z) {
		...
		this.rdm = new Random();
		rdm.setSeed(floorCount);
		int count = DungeonGenUtils.getIntBetweenBorders(dungeon.getMinFloors(), dungeon.getMaxFloors(), rdm);
		this.floors = new StrongholdFloor[count];
		this.dunX = x;
		this.dunZ = z;
		
		for(int i = 0; i < floors.length; i++) {
			int roomCount = DungeonGenUtils.getIntBetweenBorders(dungeon.getMinRoomsPerFloor(), dungeon.getMaxRoomsPerFloor(), rdm);
