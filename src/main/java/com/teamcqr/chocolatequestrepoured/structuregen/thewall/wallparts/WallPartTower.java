...
public class WallPartTower implements IWallPart {
	...
	public void generateWall(int chunkX, int chunkZ, World world, Chunk chunk) {
		...
		if (!outerWallBlocks.isEmpty() && !outerTowerBlocks.isEmpty() && !innerBlocks.isEmpty()) {
			// Inner Obsidian core
			/*
			 * for(BlockPos pos : innerBlocks) { world.setBlockState(pos, Reference.CONFIG_HELPER.wallHasObsiCore() ? Blocks.OBSIDIAN.getDefaultState() : Blocks.STONEBRICK.getDefaultState()); }
			 */
			...
			/*
			 * for(BlockPos pos : outerWallBlocks) { world.setBlockState(pos, Blocks.STONEBRICK.getDefaultState()); }
			 */
			...
			/*
			 * for(BlockPos pos : outerTowerBlocks) { world.setBlockState(pos, Blocks.STONE.getDefaultState().withProperty(BlockStone.VARIANT, BlockStone.EnumType.ANDESITE_SMOOTH)); }
			 */
