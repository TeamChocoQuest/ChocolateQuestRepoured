...
public abstract class AbstractSpellCastingItem extends Item {
	...
	public int getMaxItemUseDuration(ItemStack stack) {
		if (this.getCurrentSpell(stack) != null) {
			return Math.abs(Math.round(this.getCurrentSpell(stack).getCastingTime() * this.getCastingSpeedModifier()));
		}
		return 20;
	}
	...
	public ItemStack onItemUseFinish(ItemStack stack, World worldIn, EntityLivingBase caster) {
		if (!worldIn.isRemote && this.getCurrentSpell(stack) != null) {
			final int castingMana = Math.round(this.getCurrentSpell(stack).getSpellCosts() * this.getManaCostModifier() * this.getManaCostModifier(caster));
